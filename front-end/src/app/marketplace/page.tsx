...
export default function Marketplace() {
  ...
  useEffect(() => {
    ...
    if (location) filtered = filtered.filter((ad) => (ad.location?.toLowerCase() || "").includes(location.toLowerCase()));
    if (searchQuery) filtered = filtered.filter((ad) => (ad.title?.toLowerCase() || "").includes(searchQuery.toLowerCase()));
    setFilteredAds(filtered);
  }, [ads, category, condition, location, searchQuery]);

  const handleViewProduct = (product: Ad) => {
    setProduct({ ...product, id: product._id });
    router.push("/products");
  };

  const startChat = (sellerId: string, productId: string) => {
    if (!isLoaded) return console.warn("🕒 Clerk is still loading...");
    if (user?.id && sellerId) {
      const room = [user.id, sellerId].sort().join("___");
      router.push(`/messenger/${room}?product=${productId}`);
    } else {
            ...
            {filteredAds.map((item) => (
              ...
              >
                ...
                <CardContent className="p-4 flex flex-col flex-1">
                  ...
                  <p className="text-[#92DCE5] font-bold mt-2 dark:text-[#92DCE5]">${item.price}</p>
                  {item.location && (
                    <p className="text-sm text-gray-500 mt-1 dark:text-gray-400">📍 {item.location}</p>
                  )}
